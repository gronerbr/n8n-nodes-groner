...
export async function getContactProperties(this: ILoadOptionsFunctions): Promise<Array<{ name: string; value: string }>> {
	const credentials = await this.getCredentials('gronerApi');
	const tenant = credentials.tenant as string;

	const url = `https://${tenant}.api.groner.app/api/Lead/Propriedades`;
